...
class AppController:
    ...
    def extract_features(self):
        ...
        try:
            ...
            columns = self.data_processor.data.select_dtypes(include=[np.number]).columns.tolist()
    ...
    def run_data_mining_algorithm(self):
        ...
        try:
            ...
            if "聚类" in algorithm_type:
                ...
                elif "One-Class SVM" in method:
                    nu = float(self.window.nu_edit.text())
                    kernel = self.window.kernel_combo.currentText()
                    success, result = self.data_miner.detect_anomalies_one_class_svm(nu, kernel)
                elif "LOF" in method:
                    n_neighbors = int(self.window.n_neighbors_edit.text())
                    success, result = self.data_miner.detect_anomalies_lof(n_neighbors)
                else:
                    self.show_error_message("错误", f"不支持的异常检测方法: {method}")
                    return

                if success:
                    anomaly_count = np.sum(result['anomalies'])
                    self.show_info_message("成功", f"异常检测完成，共发现{anomaly_count}个异常")
                else:
                    self.show_error_message("异常检测错误", result)

            elif "预测" in algorithm_type:
                # 预测建模
                model_type = self.window.model_type_combo.currentText()
                test_size = float(self.window.test_size_edit.text())

                params = {}  # 根据界面设置获取参数

                success, result = self.data_miner.train_regression_model(model_type, test_size, params)
                if success:
                    metrics = result['metrics']
                    message = f"模型训练完成\nRMSE: {metrics['rmse']:.4f}\nR²: {metrics['r2']:.4f}"
                    self.show_info_message("成功", message)
                else:
                    self.show_error_message("模型训练错误", result)

            self.window.statusBar().showMessage(f"{algorithm_type}算法运行完成")

        except Exception as e:
            self.show_error_message("算法运行错误", str(e))
    ...
    def evaluate_model(self):
        """评估模型"""
        try:
            if self.data_miner.model is None:
            ...
            self.model_evaluator.set_data(
                ...
                self.data_processor.data[self.window.target_column_edit.text()]
            )
            ...
            # 评估模型
            is_classification = self.window.is_classification_radio.isChecked()

            if is_classification:
                success, result = self.model_evaluator.evaluate_classification_model()
                if success:
                    metrics = result['metrics']
                ...
                if success:
                    ...
                    # 绘制预测对比图
                    self.model_evaluator.plot_prediction_comparison()
                else:
                    self.show_error_message("评估错误", result)

            self.window.statusBar().showMessage("模型评估完成")
